...
### 特徴
- **LINE風UI**: メッセージの送受信を直感的なバブルUIで表示。最初の発言者を基準に右寄せ（黄緑色）と左寄せ（パステルカラー）に色分けし、会話の流れを視覚的に分かりやすくしました。
- **エージェント識別**: 送信元エージェントごとに一貫した色が割り当てられ、誰の発言かひと目で分かります。
- **自動更新**: 15秒ごとのポーリングにより、新しいメッセージがあれば自動的に画面に反映されます。
- **クイックナビゲーション**: 画面上部のリンクから、チャットの最上部（TOP）または最下部（BOTTOM）へ瞬時に移動できます。
- **チーム永続化**: 最後に選択したチームが保存され、次回アクセス時に自動的にロードされます。
- **ポータビリティ**: Goの単一バイナリで動作し、外部依存ライブラリなしで起動可能です。

### インストールと起動
[Releases](https://github.com/utenadev/agmsg-viewer/releases) からビルド済みバイナリを入手するか、ソースからビルドして実行します。
